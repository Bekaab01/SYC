<?php
// Turn off error reporting to prevent HTML output in file downloads
error_reporting(0);
...
// Always construct the full path correctly - stored_path contains SYCID/filename.ext
...
// Check if file exists, is a file, and not a directory
...
// For images, try to detect MIME type using PHP's built-in function
if (function_exists('mime_content_type') && (strpos($mime_type, 'image/') === 0 || $mime_type === 'application/octet-stream')) {
...
if (isset($_GET['download'])) {
    ...
    // Display inline - remove filename to prevent forced download
    header('Content-Type: ' . $mime_type);
    header('Content-Disposition: inline');
}

// Additional headers for better browser handling
header('Content-Length: ' . filesize($file_path));
header('Cache-Control: private, max-age=0, must-revalidate');
header('Pragma: public');

// For images and PDFs, add specific headers to encourage inline display
if (strpos($mime_type, 'image/') === 0 || $extension === 'pdf') {
...
// Use readfile for better performance and memory usage
