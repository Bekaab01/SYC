...
// Spot Your Cargo Transitor Dashboard - Complete logistics management interface
...
if (isset($_GET['action']) && $_GET['action'] === 'welcome') {
    ...
    echo json_encode([
        'message' => 'Welcome to the Spot Your Cargo Transitor Dashboard, ' . $welcome_name . '!'
    ]);
...
try {
    ...
    if (!empty($shipment_filters['search'])) {
        ...
        $shipments_query .= " AND (s.shipment_id LIKE ? OR s.origin_geo LIKE ? OR s.dest_geo LIKE ?)";
        $count_query .= " AND (s.shipment_id LIKE ? OR s.origin_geo LIKE ? OR s.dest_geo LIKE ?)";
...
<head>
    ...
    <title>Spot Your Cargo - Transitor Dashboard</title>
                            ...
                            <tbody>
                                <?php foreach (array_slice($shipments, 0, 5) as $shipment): ?>
                                    <tr>
                                        ...
                                        <td><?php echo htmlspecialchars($shipment['origin_geo']); ?></td>
                                        <td><?php echo htmlspecialchars($shipment['dest_geo']); ?></td>
